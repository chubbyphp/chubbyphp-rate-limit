...
final class RateLimitInfoTest extends TestCase
{
    ...
    #[DataProvider('provideGetRetryAfterCases')]
    public function testGetRetryAfter(int $reset, int $expectedRetryAfter): void
    {
        self::assertSame($expectedRetryAfter, (new RateLimitInfo(10, 0, $reset))->getRetryAfter());
    }

    /**
     * @return iterable<string, array{0: int, 1: int}>
     */
    public static function provideGetRetryAfterCases(): iterable
    {
        yield 'within the last second: at least a second' => [0, 1];
