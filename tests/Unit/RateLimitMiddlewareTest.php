...
final class RateLimitMiddlewareTest extends TestCase
{
    ...
    public function testWithKeyExceeded(): void
    {
        ...
        try {
            ...
            self::assertSame('https://datatracker.ietf.org/doc/html/rfc6585#section-4', $e->getType());
            self::assertSame(429, $e->getStatus());
            self::assertSame('Too Many Requests', $e->getTitle());
            self::assertSame([
                'type' => 'https://datatracker.ietf.org/doc/html/rfc6585#section-4',
                'status' => 429,
                'title' => 'Too Many Requests',
                'detail' => 'Limit of 10 requests reached for GET /resource, retry in 3 seconds',
                'instance' => '/resource',
                'limit' => 10,
                'remaining' => 0,
                'reset' => 3,
    ...
    public function testWithKeyExceededWithoutPathUsesTheRootAndDropsTheQueryString(): void
    {
        $builder = new MockObjectBuilder();

        $request = new ServerRequest('POST', 'https://example.com?token=secret');
        ...
        /** @var LimiterInterface $limiter */
        $limiter = $builder->create(LimiterInterface::class, [
            new WithReturn('consume', [1], new RateLimit(0, new \DateTimeImmutable('2026-01-01T12:00:03+00:00'), false, 10)),
        ]);
        ...
        try {
            ...
            self::assertSame('Limit of 10 requests reached for POST /, retry in 3 seconds', $data['detail']);
            self::assertSame('/', $data['instance']);
        }
    }
    ...
    public function testWithKeyExceededWithinTheLastSecond(): void
    {
        ...
        try {
            ...
            self::assertSame('Limit of 10 requests reached for GET /resource, retry in 1 seconds', $data['detail']);
            self::assertSame(0, $data['reset']);
            self::assertSame(1, $data['retryAfter']);
        }
    }
