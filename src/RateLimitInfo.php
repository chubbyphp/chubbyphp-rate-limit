...
final readonly class RateLimitInfo
{
    ...
    /**
     * Seconds the client should wait before retrying once the limit is exceeded: the reset of the limit, but at least
     * a second (rfc 9110), as the `Retry-After` header of the `429` response.
     */
    public function getRetryAfter(): int
    {
        return max(1, $this->reset);
    }

    /**
     * The `RateLimit-*` headers of every response.
     *
