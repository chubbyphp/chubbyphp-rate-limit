 ...
 * php: ^8.3
 * [chubbyphp/chubbyphp-http-exception][9]: ^1.3.3
 * [psr/clock][10]: ^1.0
 * [psr/http-message][2]: ^1.1|^2.0
 * [psr/http-server-handler][3]: ^1.0.2
 * [psr/http-server-middleware][4]: ^1.0.2
 * [symfony/rate-limiter][6]: ^7.3|^8.0

## Suggest

 * [chubbyphp/chubbyphp-laminas-config-factory][5]: ^1.5.2
 * [chubbyphp/chubbyphp-trusted-proxy][7]: ^1.1
 * [symfony/cache][11]: ^7.3|^8.0
 * [symfony/lock][12]: ^7.3|^8.0
...
`RateLimit-Limit`, `RateLimit-Remaining` and `RateLimit-Reset` (seconds) headers, or throws a
`TooManyRequestsException` (a `429` http exception in the sense of [chubbyphp-http-exception][9]) once the limiter
rejects.
...
Once the limit is exceeded the middleware does not return a response, it throws a `TooManyRequestsException`, which
implements the `HttpExceptionInterface` of [chubbyphp-http-exception][9], so that the exception middleware of the
application (e.g. the one of [chubbyphp-framework][13] or [chubbyphp-api][14]) turns it into the `429` response and
rate limit errors look like every other error (problem json, logging, ...). The exception carries:

 * `limit`, `remaining`, `reset` (seconds) and `retryAfter` (seconds, at least `1`) as additional problem details
   within `jsonSerialize()` (and as `getRateLimitInfo()` / `getRetryAfter()`)
 * `getHeaders()`: the `RateLimit-Limit`, `RateLimit-Remaining`, `RateLimit-Reset` and `Retry-After` headers of the
   `429` response, ready to be set by the exception middleware, never part of the body
...
The shipped exception middlewares do not set headers out of an exception, so add them (in front of the exception
middleware, or within your own one):
...
final class RateLimitHeadersMiddleware implements MiddlewareInterface
{
    ...
    public function process(ServerRequestInterface $request, RequestHandlerInterface $handler): ResponseInterface
    {
        try {
            ...
            $response->getBody()->write(json_encode($e, \JSON_THROW_ON_ERROR));
